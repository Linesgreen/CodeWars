...
function orderWeight(strng: string):string {
    let numberArray : string[] | null = strng.match(/\w+/g)
    return numberArray === null ? "" :  numberArray.sort(weightSort).join(" ")
    }
function weightSort(a : string,b : string) : number {
    let aWeight:number = a.split("").reduce((sum : number, num : string) => sum += Number(num), 0);
    let bWeight:number = b.split("").reduce((sum : number, num : string) => sum += Number(num),0);
    if (aWeight === bWeight) {
        if(a < b) {return  -1}
        if(a > b) {return 1}
        return 0
